/**
 * Gaea Skill Generator — LLM-driven MCP server package generation.
 *
 * Takes workflow records (tool call sequences) or natural language descriptions
 * and generates a complete, installable MCP server package in ~/gaea_skills/.
 */
import fs from 'fs';
...
const GENERATE_PROMPT = `You are a Skill SDK generator for Gaea. Your job is to create an MCP (Model Context Protocol) server tool with a fully implemented, executable handler function.

## Context
Gaea has learned a repeatable workflow by observing tool execution patterns. You need to encode this knowledge as a standalone MCP tool with REAL, EXECUTABLE code.
 ...
 * Auto-generated Gaea Skill: {skillName}
 * Generated at: {timestamp}
 * Generator: Gaea Skill SDK
 */
...
async function main() {
  ...
  console.error('[Gaea Skill] {skillName} ready');
}

main().catch((err) => {
  console.error('[Gaea Skill] Fatal:', err);
  process.exit(1);
});
`;
...
async function validateSkillRuntime(skillDir: string): Promise<{ valid: boolean; error?: string }> {
  return new Promise((resolve) => {
    const child = exec(
      'npx tsx index.ts',
      { timeout: 15000, cwd: skillDir, env: { ...process.env, GAEA_SKILL_SMOKE_TEST: '1' } },
