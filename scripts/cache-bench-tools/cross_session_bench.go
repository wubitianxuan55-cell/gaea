...
// cross_session_bench 测量同一目录下两次 gaeaW run 的跨会话缓存命中率
...
func main() {
	v14Bin := flag.String("v14", "", "path to V1.4 gaeaW binary")
	v15Bin := flag.String("v15", "", "path to V1.5 gaeaW binary")
...
go 1.21
`)
	writeFile(projectDir, "gaeaW.toml", fmt.Sprintf(`
default_model = "%s"
