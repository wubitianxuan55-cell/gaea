...
func TestParseGrepLine_PathWithColon(t *testing.T) {
    // 路径中包含冒号（Windows 盘符）
    file, line, text := parseGrepLine(`D:\AI\gaeaW\main.go:1: package main`)
    if file != `D:\AI\gaeaW\main.go` || line != 1 || text != "package main" {
        t.Errorf("got (%q, %d, %q)", file, line, text)
    }
}
...
```
cd gaeaW && go test ./internal/agent/... -v -count=1
```

### Step 5-2: 构建检查

```
cd gaeaW && go build -ldflags "-s -w" -o bin/gaeaW.exe ./cmd/gaeaW
go vet ./internal/...
```
