...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `gaeaW chat`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	Ready:           "已就绪",
	GetStarted:      "开始使用",
	StepScaffold:    "生成 gaeaW.toml",
	StepSetKey:      "设置 API key",

	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `gaeaW chat`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `gaeaW setup`。",
	StepSetKeyHint: "执行 export DEEPSEEK_API_KEY=… 或写入 .env",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "gaeaW help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `gaeaW chat` 开一个新的",
	...
	OutputStyleHint:   "在 gaeaW.toml 设置 agent.output_style 即可启用（下次会话生效）",
	...
	SlashMCPNone:       "没有配置 MCP 服务器 — 在 gaeaW.toml 加一个 [[plugins]] 条目",
	...
	ListHooksNone:       "无生效 hooks — 在 .gaeaW/settings.json（项目，需信任后）或 ~/.gaeaW/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 gaeaW.toml（[[plugins]]）或项目 .mcp.json 中添加",

	SelectProvidersLabel:  "选择要启用的 provider",
	EnterAPIKeysHeader:    "输入 API key（回车跳过、稍后写入 .env）：",
	MissingKeyIntro:       "gaeaW.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API key（export DEEPSEEK_API_KEY=... 或写入 .env），然后运行 `gaeaW run \"你的任务\"`。",
	ConfirmReconfigureFmt: "%s 已存在。重新配置并覆盖？",
	KeepingExisting:       "保留原配置不变。",
	NotOverwritingFmt:     "%s 已存在，不覆盖",

	UnknownCommandFmt: "未知命令 %q",
	UsageRunHint:      "用法：gaeaW run [--model NAME] <task>",
	ErrorPrefix:       "错误：",
...
用法：
  ...
  gaeaW run  [--model NAME] [--max-steps N] <task>   执行单次任务后退出
  gaeaW serve [--model NAME] [--addr HOST:PORT]      通过 HTTP+SSE 提供会话（浏览器客户端在 /）
  gaeaW setup [path]                                 交互式配置向导；生成 gaeaW.toml（及 .env）
  gaeaW mcp <add|remove|list>                        管理 gaeaW.toml 里的 MCP 服务器
  gaeaW version
  gaeaW update [--repo owner/repo] [--tag vX.Y.Z] [--check]   从 GitHub 自更新
  gaeaW help

示例：
  gaeaW chat
  gaeaW chat --continue
  gaeaW run "把 main.go 里的 TODO 实现掉"
  gaeaW run --model mimo-pro "给这个函数补单元测试"
  echo "解释这段代码" | gaeaW run

配置：
  优先级：flag > ./gaeaW.toml > ~/.config/gaeaW/config.toml > 内置默认值
  密钥通过 api_key_env 从环境变量注入（如 DEEPSEEK_API_KEY）。
  运行 'gaeaW setup' 生成配置；详见 docs/SPEC.md。
`,
}
