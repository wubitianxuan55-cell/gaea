...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `gaeaW chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold gaeaW.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `gaeaW chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `gaeaW setup`.",
	StepSetKeyHint: "export DEEPSEEK_API_KEY=… or add to .env",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "gaeaW help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `gaeaW chat`",
	...
	OutputStyleHint:   "set agent.output_style in gaeaW.toml to apply one (takes effect next session)",
	...
	SlashMCPNone:       "no MCP servers configured — add a [[plugins]] entry in gaeaW.toml",
	...
	ListHooksNone:       "hooks: none active — configure in .gaeaW/settings.json (project, after trust) or ~/.gaeaW/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in gaeaW.toml ([[plugins]]) or a project .mcp.json",

	SelectProvidersLabel:  "Select providers to enable",
	EnterAPIKeysHeader:    "Enter API keys (Enter to skip and set later in .env):",
	MissingKeyIntro:       "gaeaW.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (export DEEPSEEK_API_KEY=... or add to .env), then run `gaeaW run \"your task\"`.",
	...
	UsageRunHint:      "usage: gaeaW run [--model NAME] <task>",
	ErrorPrefix:       "error:",
...
Usage:
  ...
  gaeaW run  [--model NAME] [--max-steps N] <task>   run one task and exit
  gaeaW serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  gaeaW setup [path]                                  interactive config wizard; writes gaeaW.toml (+ .env)
  gaeaW mcp <add|remove|list>                         manage MCP servers in gaeaW.toml
  gaeaW version
  gaeaW update [--repo owner/repo] [--tag vX.Y.Z] [--check]   self-update from GitHub
  gaeaW help

Examples:
  gaeaW chat
  gaeaW chat --continue
  gaeaW run "implement the TODOs in main.go"
  gaeaW run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | gaeaW run

Configuration:
  Resolution: flag > ./gaeaW.toml > ~/.config/gaeaW/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'gaeaW setup' to scaffold a config; see docs/SPEC.md.
`,
}
