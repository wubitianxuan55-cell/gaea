...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — gaeaW setup
	StepSetKey      string // step 2 command label

	// `gaeaW init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // gaeaW chat step desc
	StepRunDesc    string // gaeaW run step desc
	HelpFooter     string // dim footer linking to gaeaW help
	...
	WroteFileFmt          string // "Wrote %s" — used for gaeaW.toml and .env both
	...
	UsageRunHint      string // "usage: gaeaW run [--model NAME] <task>"
