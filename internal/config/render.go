...
// RenderTOML renders the config as annotated TOML in the `gaeaW setup` house style:
...
func RenderTOML(c *Config) string {
	...
	b.WriteString("# Resolution order: flag > ./gaeaW.toml > ~/.config/gaeaW/config.toml > built-in defaults.\n")
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"gaeaW-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `gaeaW setup`
