...
// flag > project ./gaeaW.toml > user ~/.config/gaeaW/config.toml > built-in defaults.
...
// (default true) lets gaeaW fetch the CodeGraph runtime into its cache on first
...
// the project roots (.gaeaW/.agents/.claude under the workspace) and the
...
type AgentConfig struct {
	...
	// .gaeaW/output-styles/<name>.md). Empty = the unmodified prompt.
...
// gaeaW.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
...
const DefaultSystemPrompt = `你是 gaeaW，一个土壤修复工程办公专用AI助手。所有思考和输出必须使用中文。
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `gaeaW run` autonomous (no TTY → ask
		// resolves to allow) while `gaeaW chat` prompts before writers. Users add
...
func Load() (*Config, error) {
	...
	if err := mergeFile(cfg, "gaeaW.toml"); err != nil {
		return nil, err
	}
	// Claude Code's .mcp.json (project root) is read last and merged into
	// [[plugins]], so a server configured for Claude works here unchanged.
	// gaeaW.toml wins on a name collision (see mergeMCPJSON).
...
// LoadForEdit returns a config to seed the `gaeaW setup` wizard when reconfiguring:
...
func userConfigPath() string {
	...
	return filepath.Join(dir, "gaeaW", "config.toml")
}

// UserConfigPath is the user-global config file (~/.config/gaeaW/config.toml),
...
func ArchiveDir() string {
	...
	return filepath.Join(dir, "gaeaW", "archive")
}

// SessionDir is where chat sessions are persisted (one .jsonl per session).
// Used by `gaeaW chat --continue` / `--resume` to find the recent ones. Empty
...
func SessionDir() string {
	...
	return filepath.Join(dir, "gaeaW", "sessions")
}

// WorkspaceSessionDir returns the workspace-scoped session directory under
// cwd/.gaeaW/sessions/. Sessions are isolated per workspace so switching
...
func WorkspaceSessionDir(cwd string) string {
	...
	return filepath.Join(cwd, ".gaeaW", "sessions")
}

// MemoryUserDir returns the gaeaW user config root (…/gaeaW), under which
...
func MemoryUserDir() string {
	...
	return filepath.Join(dir, "gaeaW")
}

// ConventionDirs are the parent directories scanned for agent assets (skills,
// commands), in canonical-first order. .gaeaW is ours; .agents / .agent /
...
// .gaeaW/settings.json (see internal/hook).
var ConventionDirs = []string{".gaeaW", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .gaeaW ends up the
...
// on a name clash. Order: home-dir convention dirs (~/.claude/commands … ~/.gaeaW/commands),
// the legacy XDG user dir (~/.config/gaeaW/commands), then the project's
// convention dirs (.claude/commands … .gaeaW/commands). Scanning the .claude /
...
func CommandDirs() []string {
	...
	if dir, err := os.UserConfigDir(); err == nil {
		dirs = append(dirs, filepath.Join(dir, "gaeaW", "commands")) // legacy XDG user dir
	}
...
func SourcePath() string {
	if _, err := os.Stat("gaeaW.toml"); err == nil {
		return "gaeaW.toml"
	}
	if uc := userConfigPath(); uc != "" {
