...
// separate from the `gaeaW setup` wizard (cli) so a GUI can apply one setting at a
...
// half-written gaeaW.toml that fails to parse on next load. Parent directories
...
func (c *Config) SaveTo(path string) error {
	...
	tmp, err := os.CreateTemp(dir, ".gaeaW.*.toml.tmp")
	if err != nil {
...
// (SourcePath), or to ./gaeaW.toml when none exists yet — the conventional
