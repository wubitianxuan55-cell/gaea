...
// Trust gates project hooks. A project's .gaeaW/settings.json can run
...
// root. The trust flag lives in user-global state (~/.gaeaW/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.gaeaW/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.gaeaW.
...
// TrustPath is ~/.gaeaW/trust.json (homeDir overrides ~).
