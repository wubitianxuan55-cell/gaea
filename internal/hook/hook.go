...
// (.gaeaW/settings.json, only when the project is trusted) and a global
// (~/.gaeaW/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".gaeaW"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.gaeaW/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.gaeaW/settings.json.
