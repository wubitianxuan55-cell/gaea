...
// buildExamplePlugin compiles cmd/gaeaW-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "gaeaW-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "gaeaW/cmd/gaeaW-plugin-example").CombinedOutput()
	if err != nil {
