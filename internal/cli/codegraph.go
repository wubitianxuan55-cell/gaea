...
// codegraphCommand backs `gaeaW codegraph` — managing the CodeGraph
// code-intelligence runtime that gaeaW otherwise fetches lazily on first use.
...
func codegraphStatus() int {
	...
	if p, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-13s %s\n", "resolved:", "(not installed — run `gaeaW codegraph install`)")
	}
	return 0
}

func codegraphUsage() {
	fmt.Print(`gaeaW codegraph — manage the CodeGraph code-intelligence runtime

Usage:
  gaeaW codegraph install   download + cache the runtime for this platform
  gaeaW codegraph status    show config, cache dir, and resolved launcher
