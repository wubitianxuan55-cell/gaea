...
// mcp.go holds the MCP server-management surface shared by the `gaeaW mcp`
...
// mcpCommand implements `gaeaW mcp <add|remove|list>`. It edits config only
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: gaeaW mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to gaeaW.toml).

Usage:
  gaeaW mcp list
  gaeaW mcp add <name> <command> [args...]        stdio server
  gaeaW mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  gaeaW mcp add <name> --sse  <url>               remote (legacy SSE)
  gaeaW mcp remove <name>
...
Examples:
  gaeaW mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  gaeaW mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
