// Package cli implements gaeaW's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `gaeaW init` isn't a dead end.
		return initHint()
	case "acp":
...
func runServe(args []string) int {
	...
	fmt.Printf("gaeaW serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// setupConfig runs the configuration wizard (the `gaeaW setup` command),
// writing gaeaW.toml (+ .env). Project memory is a separate concern — the
...
// initHint handles `gaeaW init`. Unlike a config scaffold, project memory is
...
// (and to `gaeaW setup` for config) so the verb isn't a dead end.
func initHint() int {
	fmt.Println(i18n.M.InitHint)
	return 0
}

// interactiveSetup runs the setup wizard, then writes gaeaW.toml (plus .env for any
...
func interactiveSetup(path string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("gaeaW")),
		"",
...
// edit gaeaW.toml afterward — keeping first-run a single decision instead of two.
...
// not exist in config because users editing gaeaW.toml deal with SKU names
...
// are appended — so re-running `gaeaW setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	if src == "" && isInteractive() {
		if rc := interactiveSetup("gaeaW.toml"); rc != 0 {
			return rc
		}
	...
	// gaeaW.toml exists and parses on a terminal: go into chat. If any enabled
	...
	if src == "" {
		step("gaeaW setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("gaeaW chat", i18n.M.StepChatDesc)
	step(`gaeaW run "task"`, i18n.M.StepRunDesc)
