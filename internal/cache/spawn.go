...
const subagentExplorePrefix = `你是 gaeaW 的代码探索子代理。你的任务是在代码库中进行深入调查。
...
const subagentResearchPrefix = `你是 gaeaW 的外部调研子代理。你的任务是从外部资源收集信息。
...
const subagentReviewPrefix = `你是 gaeaW 的代码审查子代理。你的任务是审查代码变更。
...
const subagentSecurityPrefix = `你是 gaeaW 的安全审计子代理。你的任务是审计代码的安全性。
