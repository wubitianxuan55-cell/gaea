...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.gaeaW/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.gaeaW/skills/ (this workspace only); 'global' writes to ~/.gaeaW/skills/ (every project)."
	}
