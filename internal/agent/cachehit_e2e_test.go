...
import (
	...
	"gaeaW/internal/provider/openai"
	"gaeaW/internal/tool"
)
...
func TestCacheHitSurvivesTooSmallWindow(t *testing.T) {
	...
	// gaeaW V5.0: no auto-compaction-paused guard — the simple truncation
	// strategy naturally limits itself (can't truncate fewer than minRecentKeep).
	if paused {
		t.Log("auto-compaction paused notice emitted (not expected in gaeaW)")
	}
