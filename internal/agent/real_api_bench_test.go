...
import (
	...
	_ "gaeaW/internal/provider/openai" // register openai provider
	"gaeaW/internal/tool"
)
...
func TestRealAPICache10Rounds(t *testing.T) {
	...
	fmt.Println()
	fmt.Println("═══════════════════════════════════════════════════════════════")
	fmt.Println("  gaeaW V5.22  真实 DeepSeek API 10 轮缓存测试")
	...
	fmt.Printf("  本测试系统提示: ~20 词（极简模式，非完整 gaeaW 配置）\n")
	...
	fmt.Println()
	fmt.Println("  💡 完整 gaeaW 配置的缓存测试请运行:")
