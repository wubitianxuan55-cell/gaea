...
// TestRealAPIFullSetup10Rounds 完整 gaeaW 配置 10 轮真实 API 缓存测试。
...
func TestRealAPIFullSetup10Rounds(t *testing.T) {
	...
	// 完整系统提示词（模拟 gaeaW 实际使用的 L1）
	systemPrompt := `You are gaeaW, an AI coding agent powered by DeepSeek V4.

## Identity
- You are a desktop AI coding assistant running inside the gaeaW application.
...
- When a task is done, summarize what was changed.`
	...
	runtimePrompt := `## Project: gaeaW
- Language: Go 1.22
- Build: go build -o bin/gaeaW.exe ./cmd/gaeaW
...
Answer coding questions about Go concepts concisely in Chinese.`
	...
	fmt.Println()
	fmt.Println("══════════════════════════════════════════════════════════════════")
	fmt.Println("  gaeaW V5.22  完整配置 真实 DeepSeek API 10 轮缓存测试")
