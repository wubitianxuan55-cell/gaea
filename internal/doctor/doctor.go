// Package doctor provides a diagnostic engine that checks gaeaW's health:
...
// `gaeaW doctor` CLI command and the `doctor` model-facing tool.
package doctor
...
func checkConfig(deps Deps) Item {
	...
	if deps.ProviderName == "" {
		return Item{Name: "配置", Status: Fail, Detail: "未设置默认模型", Advice: "运行 gaeaW setup 配置模型"}
	}
