// Package update provides self-update for gaeaW. It fetches the latest
// release from a GitHub repository and replaces the running binary.
//
// Usage:
//
//	err := update.Self(context.Background(), "gaeaWX/gaeaW", "v7.3.0")
...
func Self(ctx context.Context, repo, currentVersion, releaseTag string) error {
	...
	if releaseTag == "" && compareVersions(currTag, latestTag) >= 0 {
		fmt.Fprintf(os.Stderr, "✓ gaeaW %s is up to date (latest: %s)\n", currentVersion, latest.TagName)
		return nil
	}
	...
	// 4. Download
	fmt.Fprintf(os.Stderr, "⬇ downloading gaeaW %s (%s)...\n", latest.TagName, assetName)
	...
	if runtime.GOOS == "windows" {
		...
		if err := os.Rename(exe, old); err != nil {
			_ = os.Remove(tmp)
			return fmt.Errorf("update: rename current → .old: %w (try closing gaeaW first)", err)
		}
		...
		fmt.Fprintf(os.Stderr, "✓ updated to gaeaW %s (old binary saved as %s)\n", latest.TagName, old)
	} else {
		if err := os.Rename(tmp, exe); err != nil {
			_ = os.Remove(tmp)
			return fmt.Errorf("update: rename: %w", err)
		}
		fmt.Fprintf(os.Stderr, "✓ updated to gaeaW %s\n", latest.TagName)
	}
	return nil
}
...
func assetName() string {
	...
	return fmt.Sprintf("gaeaW-%s-%s%s", runtime.GOOS, arch, ext)
}
