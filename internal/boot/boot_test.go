...
import (
	...
	// cmd/gaeaW's main does; without them Build sees an empty provider
...
func TestBuildFoldsProjectMemoryIntoSystemPrompt(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("APPDATA", dir) // isolate from user-global config (~/.config/gaeaW/)
	t.Chdir(dir)

	writeFile(t, dir, "gaeaW.toml", `
...
api_key_env = "TIANXUAN_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".gaeaW/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
[[plugins]]
name = "missing"
command = "gaeaW-missing-mcp-binary"
`)
