...
if cfg.Agent.Effort != "" { entry.Effort = cfg.Agent.Effort }
	...
	// resolves "ask" to allow — preserving `gaeaW run` autonomy — while deny
	...
	archiveDir := filepath.Join(cwd, ".gaeaW", "archive")
	...
	// Custom slash commands (.gaeaW/commands + user dir). Best-effort: a malformed
	...
	cacheDir := filepath.Join(cwd, ".gaeaW", "cache")
