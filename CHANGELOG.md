...
- **成本库（costdb）全量优化**：修复 3 个并发安全 Bug（`Save()` 写锁、`SetData()` 无锁、CSV 导入竞态），新增 5 个 O(1) 哈希索引，所有 `find*`/`Delete*`/`Update*` 从 O(n) 线性扫描降为 O(1) 查找，`Estimate`/`RegionFactor`/`RegionCompare` 同步优化。swap-with-last 删除模式。自动索引重建。64 个单元测试全部 PASS（`internal/costdb/`）
...
- CLI：`release/v0.13.2/gaeaW.exe`
- 桌面端：`release/v0.13.2/gaeaW-desktop.exe`
