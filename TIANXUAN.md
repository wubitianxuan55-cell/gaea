...
### 构建产物
- release/v8.23.0/gaeaW.exe (16MB)
- release/v8.23.0/gaeaW-desktop.exe (16MB)
...
📦 CCR可逆压缩: .gaeaW/ccr/ + retrieve 工具, hash取回原文
...
### 发布产物
📦 gaeaW.exe (16.6MB, CGO_ENABLED=0)
📦 gaeaW-desktop.exe (16.2MB, Wails v2.12.0)
...
gaeaW 是一个面向 DeepSeek V4 的极简 Coding Agent。单 Go 二进制，零外部依赖。
核心目标：极低成本、极快响应。
...
```
# 构建 CLI（发布版）
set GOOS=windows&& set GOARCH=amd64&& go build -ldflags="-s -w" -o release/vX.Y.Z/gaeaW.exe ./cmd/gaeaW/

# 构建桌面端
cd desktop && wails build
cp build/bin/gaeaW-desktop.exe ../release/vX.Y.Z/
...
- Config: gaeaW.toml, secrets in .env
