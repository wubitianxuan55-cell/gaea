...
const (
	...
	manifestFallback    = "https://github.com/wubitianxuan55-cell/gaea/releases/latest/download/latest.json"
	defaultDownloadPage = "https://github.com/wubitianxuan55-cell/gaea/releases/latest"
...
func applyLinux(targz []byte) error {
	bin, err := extractBinary(targz, "gaeaW-desktop")
	if err != nil {
		return err
	}
...
func applyWindows(installer []byte) error {
	f, err := os.CreateTemp("", "gaeaW-update-*.exe")
	if err != nil {
		return err
	}
	name := f.Name()
