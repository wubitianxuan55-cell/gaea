...
import {
  ...
  loadWorkspaceFileTreePanelWidth,
  saveWorkspaceFileTreePanelWidth,
} from "./useLayoutSizes";
...
export function useWorkspacePanel(effectiveSidebarWidth: number, viewportWidth: number) {
  ...
  const [workspaceFileTreePanelWidth, setWorkspaceFileTreePanelWidth] = useState(
    loadWorkspaceFileTreePanelWidth,
  );
  ...
  const startWorkspacePanelResize = useCallback(
    (e: ReactPointerEvent<HTMLButtonElement>) => {
      ...
      const clampFn = workspacePreviewModeActive
        ? clampWorkspacePanelWidth
        : clampWorkspaceFileTreePanelWidth;
    ...
    (e: KeyboardEvent<HTMLButtonElement>) => {
      if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
        ...
        setSavedWorkspacePanelWidth(
          effectiveWorkspacePanelWidth + (e.key === "ArrowLeft" ? 16 : -16),
        );
      } else if (e.key === "Home") {
        e.preventDefault();
        setSavedWorkspacePanelWidth(
  ...
  return {
    ...
    setWorkspaceFileTreePanelWidth,
    setWorkspacePanelResizing,
    setWorkspacePanelMaximized,
    setWorkspacePreviewModeActive,
    setSavedWorkspacePanelWidth,
