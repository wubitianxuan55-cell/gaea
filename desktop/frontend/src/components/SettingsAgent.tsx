...
export function AgentSection({ s, busy, apply }: SectionProps) {
  ...
  const dirty =
    temp !== String(s.agent.temperature) ||
    steps !== String(s.agent.maxSteps) ||
  ...
  return (
    <section className="mb-3">
      ...
      <div className="flex items-center gap-3 mb-2.5">
        <label className="text-fg-dim text-[13px] w-[80px] shrink-0">
          ...
          {skillsOpen && (
            <div className="mt-2 space-y-2 pl-4 border-l-2 border-border-soft">
              {s.subagentSkills.map((skill: string) => {
                ...
                const inheritText = globalRef
                  ? `继承全局: ${globalRef}`
                  : t("settings.subagentInherit");
                const skillLabel = skillRef || inheritText;
                return (
                  <div key={skill} className="flex items-center gap-3">
                    <label className="text-fg-dim text-[12px] w-[100px] shrink-0 font-mono">
                      {skill}
                    </label>
                    <div className="flex-1">
                      <ModelSwitcher
                        ...
                        onPick={(ref: string) =>
                          void apply(() => app.SetSubagentModelForSkill(skill, ref))
                        }
                      />
                    </div>
                  </div>
      ...
      <div className="flex items-center gap-3 mb-2.5">
        ...
        <input
          className="w-[70px] bg-bg-soft border border-border-soft rounded-md text-fg text-[13px] px-2.5 py-1.5 outline-none placeholder:text-fg-faint focus:border-accent text-center"
          value={temp}
      ...
      <textarea
        className="w-full bg-bg-soft border border-border-soft rounded-md text-fg text-[13px] p-2.5 outline-none resize-y min-h-[120px] focus:border-accent"
        value={prompt}
      ...
      <div className="flex gap-2 mt-2">
        <button
          ...
          disabled={busy || !dirty}
          onClick={() =>
            void apply(() => app.SetAgentParams(Number(temp) || 0, Number(steps) || 0, prompt))
          }
        >
