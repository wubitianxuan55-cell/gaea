    ...
    <div className="relative inline-flex">
      ...
      {open && (
        <>
          ...
          <div
            className="absolute bottom-full left-0 mb-1 z-50 bg-bg-elev-2 border border-border rounded-lg min-w-[200px] max-h-[240px] overflow-y-auto"
          ...
          >
            ...
            {jobs.map((j) => (
              <div
                className="flex items-center gap-2 px-3 py-1.5 text-[12px] hover:bg-bg-elev border-b border-border-soft last:border-0"
                key={j.id}
                role="option"
              >
...
export function ContextBar({
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="text-fg-dim font-mono tabular-nums text-[9px] shrink-0 w-7 text-right">
        {pct}%
      </span>
      <span className="text-fg-faint font-mono tabular-nums text-[8px] shrink-0">
...
export const StatusBar = memo(function StatusBar({
  ...
  const sessionCost =
    totalPromptTokens > 0
      ? calcCost(sessionHit, p.cacheHit) +
        calcCost(sessionMiss, p.input) +
        calcCost(sessionCompl, p.output)
      : 0;
  ...
  const cacheColor =
    sessionRate >= 80 ? "text-ok" : sessionRate >= 50 ? "text-warning" : "text-err";
  const cacheBadge =
    sessionRate >= 80
  ...
  const nowColor =
    nowRate == null ? "" : nowRate >= 80 ? "text-ok" : nowRate >= 50 ? "text-warning" : "text-err";
  ...
  return (
    <div
      className={`flex items-center gap-2 ${barH} ${barPx} ${fontSize} bg-bg-soft border-t border-border-soft select-none shrink-0`}
      data-wails-no-drag
    >
      {/* ── 左: 连接灯 + 状态文字 ── */}
        ...
        {permLevel &&
          ...
          (() => {
            ...
            return (
              <Tooltip label={desc}>
                <span
                  className={`${fontSize} px-1.5 py-px rounded border font-medium ${colorClass}`}
                >
                  {label}
                </span>
              </Tooltip>
        ...
        {usage && sessionTotal > 0 && (
          <>
            ...
            <Tooltip
              label={
                nowRate != null
                  ? `本轮 ${nowRate}% · 会话 ${sessionRate}%`
                  : `会话命中率 ${sessionRate}%`
              }
            >
              <span
                className={`${fontSize} font-semibold px-1.5 py-px rounded border ${cacheColor} ${cacheBadge}`}
              >
                缓存 {sessionRate}%
            ...
            <Tooltip label="提示 Token">
              <span className="text-fg-dim tabular-nums">
                提{fmtTokens(usage?.promptTokens ?? 0)}
              </span>
            </Tooltip>
            <Tooltip label="输出 Token">
              <span className="text-fg-dim tabular-nums">
                出{fmtTokens(usage?.completionTokens ?? 0)}
              </span>
            </Tooltip>
            <Tooltip label="缓存命中">
            ...
            <Tooltip label="缓存未命中">
              <span className="text-err tabular-nums">
                ✗{fmtTokens(usage?.cacheMissTokens ?? 0)}
              </span>
            </Tooltip>
          </>
        )}
