  ...
  const onPaste = useCallback(
    (e: ClipboardEvent<HTMLTextAreaElement>) => {
      ...
      requestAnimationFrame(() => {
        ...
        if (n) {
          n.focus();
          n.selectionStart = n.selectionEnd = start + label.length;
        }
      });
    },
  ...
  return (
    <div className="pb-1.5">
      {blocks.map((block) => {
        ...
        return (
          <div
            className="mb-1 border border-border-soft rounded-lg overflow-hidden"
            key={block.label}
          >
            <div className="flex items-center gap-1.5 px-2 py-1 text-xs">
              ...
              <div className="flex items-center gap-0.5 ml-auto">
                <ActionBtn
                  ...
                  onClick={() => onTogglePreview(block.label)}
                >
                  <Eye size={13} />
                </ActionBtn>
...
function ActionBtn({
  ...
  return (
    <button
      type="button"
      className={`px-1.5 py-0.5 bg-transparent border-0 rounded text-fg-faint cursor-pointer text-[11px] transition-colors ${danger ? "hover:text-err hover:bg-bg-soft" : "hover:text-fg hover:bg-bg-soft"}`}
      title={title}
      onClick={onClick}
    >
      {children}
