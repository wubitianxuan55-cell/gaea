...
export function Composer({
  ...
  const slashQuery = useMemo(
    () => (!text.startsWith("/") || /\s/.test(text) ? null : text.slice(1).toLowerCase()),
    [text],
  );
  const slashMatches = useMemo(
    () =>
      slashQuery === null
        ? []
        : commands.filter((c) => c.name.toLowerCase().includes(slashQuery)).slice(0, 8),
    [slashQuery, commands],
  );
  ...
  const atDir = useMemo(() => {
    ...
    const s = atRaw.lastIndexOf("/");
    return s >= 0 ? atRaw.slice(0, s + 1) : "";
  }, [atRaw]);
  const atFrag = useMemo(() => {
    if (atRaw === null) return "";
    const s = atRaw.lastIndexOf("/");
    return (s >= 0 ? atRaw.slice(s + 1) : atRaw).toLowerCase();
  }, [atRaw]);
  ...
  const atMatches = useMemo(
    () =>
      atRaw === null
        ? []
        : entries.filter((e) => e.name.toLowerCase().includes(atFrag)).slice(0, 10),
  ...
  const submit = () => {
    ...
    const submitText = [paste.expandBlocks(tTrim), refs]
      .filter(Boolean)
      .join(tTrim && refs ? " " : "");
    if (displayText.trim()) {
      try {
        ...
        sessionStorage.setItem(
          INPUT_HISTORY_KEY,
          JSON.stringify(history.slice(0, MAX_INPUT_HISTORY)),
        );
      } catch {}
    }
    setHistoryIndex(-1);
  ...
  const onPaste = (e: ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    const hasImageData = Array.from(e.clipboardData.items).some((it) =>
  ...
  useEffect(() => {
    ...
    const close = (e: MouseEvent) => {
      ...
      if (workspaceAnchorRef.current?.contains(tgt) || workspaceMenuRef.current?.contains(tgt))
        return;
  ...
  const saveComposerHeight = (h: number) =>
    saveLayoutSize("composerHeight", h, clampComposerHeight);
    ...
    <div className="relative max-w-[--maxw] mx-auto">
          ...
          <div className="max-h-[280px] overflow-y-auto mb-1">
            ...
            {filteredWorkspaces.length === 0 && (
              <div className="py-4 text-fg-faint text-xs text-center">
                {t("composer.noProjectMatches")}
              </div>
            )}
          </div>
          <div className="pt-1 border-t border-border-soft">
            <button
              className="flex items-center gap-2.5 w-full px-2 py-1.5 bg-transparent border-0 rounded-lg text-left cursor-pointer text-fg-dim hover:bg-bg-soft hover:text-fg text-[13px] transition-colors"
      ...
      {attachments.length > 0 && (
        <div className="flex flex-wrap gap-1.5 px-1 pb-1.5">
          {attachments.map((a) => (
            <div
              className="flex items-center gap-1.5 pl-1.5 pr-1 py-1 bg-bg-elev-2 border border-border-soft rounded-lg text-xs"
              key={a.path}
            >
              {a.type === "image" ? (
              ...
              <span className="max-w-[120px] truncate text-fg-dim font-mono text-[11px]">
                {a.path.split("/").pop()}
              </span>
              <button
                type="button"
                className="flex items-center justify-center w-5 h-5 bg-transparent border-0 rounded text-fg-faint cursor-pointer hover:text-err hover:bg-bg-soft transition-colors"
                title="移除"
      ...
      <div
        ...
        style={{
          ...(composerHeight !== null ? { height: "var(--composer-height)" } : {}),
          ...composerCardStyle,
        }}
        ref={composerCardRef}
      >
        ...
        >
          <span className="text-accent font-mono font-semibold text-lg leading-[1.55] shrink-0 select-none">
            ›
          </span>
          <textarea
            ref={taRef}
          ...
          {running && (
            <button
              className="inline-flex items-center justify-center w-[30px] h-[30px] border-0 rounded-md cursor-pointer shrink-0 transition-all duration-[var(--dur-fast)] bg-bg-elev-2 text-err hover:bg-err hover:text-white active:scale-95"
          ...
          <button
            ...
            disabled={
              disabled ||
              pendingPaste > 0 ||
              (!text.trim() && attachments.length === 0 && (!running || queueLen === 0))
            }
            title={
              running
                ? queueLen > 0
          ...
          {cwd && (
            <div className="relative inline-flex min-w-0" ref={workspaceAnchorRef}>
              <button
                ...
                onClick={() => {
                  if (!running) setWorkspaceMenuOpen((o) => !o);
                }}
                disabled={running}
            ...
            {(["ask", "auto", "yolo"] as const).map((level) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (permLevel !== level && onSetPermLevel) onSetPermLevel(level);
                  }}
                  title={descs[level]}
                >
                  {labels[level]}
