...
function StatusGlyph({
  ...
  if (status === "error")
    return <X className={recoverable ? "text-fg-faint/60" : "text-err"} size={12} />;
...
export const ToolCard = memo(function ToolCard({
  ...
  return (
    <div
      className={`my-px rounded-md overflow-hidden border transition-colors duration-300 ${
        item.status === "error" && !item.recoverable
          ? "border-err/40"
          : item.status === "error" && item.recoverable
            ? "border-fg-faint/30"
            : item.status === "running"
              ? "border-accent/30 bg-accent/[0.02] shadow-[0_0_6px_var(--accent-soft)]"
              : item.status === "stopped"
    ...
    >
      ...
      >
        ...
        />
        <span
          className={`font-mono font-medium truncate ${item.status === "error" && !item.recoverable ? "text-err" : item.status === "error" && item.recoverable ? "text-fg-dim/60 line-through" : "text-fg"} ${compact ? "text-[11px]" : "text-[12px]"}`}
        >
          {item.name}
        </span>
        {subject && <span className={`text-fg-faint truncate ${summarySize}`}>{subject}</span>}
        {summary && <span className={`text-fg-faint italic ml-0.5 ${summarySize}`}>{summary}</span>}
        ...
        <div>
          {diffs.map((d, i) => (
            <div className={`${innerPx} ${innerPb}`} key={i}>
              {d.label && (
                <div className="text-[10px] text-fg-faint uppercase tracking-wider mb-0.5">
                  {d.label}
                </div>
              )}
              <pre className="px-3 py-2 font-mono text-[12px] leading-[1.5] overflow-auto whitespace-pre bg-bg-soft border border-border-soft rounded text-fg-dim">
          ...
          {hasOutput && (
            <div className={`${innerPx} ${innerPb}`}>
              <div className="text-[9px] text-fg-faint/60 uppercase tracking-wider mb-0.5 select-none">
                输出 · {outputLines}L
              </div>
              <pre className="px-3 py-2 font-mono text-[12px] leading-[1.5] overflow-auto whitespace-pre bg-bg-soft border border-border-soft rounded text-fg-dim">
          ...
          {item.error && !item.recoverable && (
            <div
              className={`${innerPx} py-1 text-err text-[12px] leading-snug border-t border-err/20`}
            >
              {item.error}
            </div>
          )}
          {item.error && item.recoverable && (
            <div
              className={`${innerPx} py-1 text-fg-faint/60 text-[12px] leading-snug border-t border-fg-faint/15`}
            >
              {item.error}
            </div>
          )}
