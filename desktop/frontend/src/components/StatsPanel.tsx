...
export function useStatsPersistence(
  ...
  const turnAccumRef = useRef<{
    prompt: number;
    completion: number;
    cacheHit: number;
    cacheMiss: number;
    cost: number;
  }>({ prompt: 0, completion: 0, cacheHit: 0, cacheMiss: 0, cost: 0 });
  ...
  useEffect(() => {
    ...
    setData((prev) => {
      if (prev.steps.length > 0) {
        ...
        if (
          prevStep.prompt === lastStep.promptTokens &&
          prevStep.completion === lastStep.completionTokens &&
          prevStep.cacheHit === lastStep.cacheHitTokens &&
          prevStep.cacheMiss === lastStep.cacheMissTokens &&
        ...
        <table className="w-full text-[11px] border-collapse">
          <tbody>
            <tr className="font-bold">
              <td className="py-1 text-fg" style={{ width: "28%" }}>
                {title}
              </td>
              <td className="py-1 text-right font-mono tabular-nums" style={{ width: "24%" }}>
                {tk(total.prompt)}
              </td>
              <td className="py-1 text-right font-mono tabular-nums" style={{ width: "24%" }}>
                {tk(total.completion)}
              ...
              <td colSpan={4} className="py-1 text-left">
                {t === 0 ? (
                  ...
                  <span className={`text-base font-bold tabular-nums ${hitRateColor(rate)}`}>
                    {rate.toFixed(2)}%{" "}
                    <span className="text-[10px] text-fg-faint font-normal">
                      {tk(total.cacheHit)} 命中 / {tk(total.cacheMiss)} 未命中
                    </span>
                  </span>
                )}
              </td>
      ...
      <table className="w-full text-[11px] border-collapse">
        <thead>
          <tr className="text-fg-faint border-b border-border-soft">
            <th
              className="text-left font-semibold pb-1 text-[10px] uppercase tracking-wider text-fg-faint"
              style={{ width: "28%" }}
            >
              {title}
            </th>
            <th className="text-right font-normal pb-1" style={{ width: "24%" }}>
              规划
            </th>
            <th className="text-right font-normal pb-1" style={{ width: "24%" }}>
              执行
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const pRate =
              isHitRow && planner.cacheHit + planner.cacheMiss > 0
                ? (planner.cacheHit / (planner.cacheHit + planner.cacheMiss)) * 100
                : 0;
            const eRate =
              isHitRow && executor.cacheHit + executor.cacheMiss > 0
                ? (executor.cacheHit / (executor.cacheHit + executor.cacheMiss)) * 100
                : 0;
            const sRate =
              isHitRow && sub.cacheHit + sub.cacheMiss > 0
                ? (sub.cacheHit / (sub.cacheHit + sub.cacheMiss)) * 100
                : 0;
            return (
              <tr key={row.label} className="border-b border-border-soft/50">
                ...
                {isHitRow ? (
                  <>
                    <td
                      className={`py-1 text-right font-mono tabular-nums font-bold ${hitRateColor(pRate)}`}
                    >
                      {planner.cacheHit + planner.cacheMiss > 0 ? `${pRate.toFixed(2)}%` : "—"}
                    </td>
                    <td
                      className={`py-1 text-right font-mono tabular-nums font-bold ${hitRateColor(eRate)}`}
                    >
                      {executor.cacheHit + executor.cacheMiss > 0 ? `${eRate.toFixed(2)}%` : "—"}
                    </td>
                    <td
                      className={`py-1 text-right font-mono tabular-nums font-bold ${hitRateColor(sRate)}`}
                    >
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-baseline gap-2">
                    <span className={`text-xl font-bold tabular-nums ${hitRateColor(rate2)}`}>
                      {rate2.toFixed(2)}%
                    </span>
...
function HitRateTrend({
  ...
  const dataMin = Math.min(...rates),
    dataMax = Math.max(...rates),
    spread = dataMax - dataMin || 1;
  ...
  if (spread <= 0.5) {
    ...
    step = 5;
    padding = Math.max(5, spread * 0.15);
  }
  ...
  const xLabels = [
    ...
    ...(recent.length >= 3
      ? [
          {
            at: points[Math.floor(points.length / 2)].x,
            text: `#${recent[Math.floor(recent.length / 2)].step}`,
          },
        ]
      : []),
...
function TokenTrendChart({ history }: { history: TurnRecord[] }) {
  ...
  const pYLabels: [number, string][] = [
    [0, "0"],
    [Math.round(pMax * 0.5), tkK(Math.round(pMax * 0.5))],
    [pMax, tkK(pMax)],
  ];
  const cYLabels: [number, string][] = [
    [0, "0"],
    [Math.round(cMax * 0.5), tk(Math.round(cMax * 0.5))],
    [cMax, tk(cMax)],
  ];
  const xLabels =
    recent.length >= 3
      ? [
          { at: pPoints[0].x, text: `#${recent[0].turn}` },
          { at: pPoints[pPoints.length - 1].x, text: `#${recent[recent.length - 1].turn}` },
        ]
      : [];
  return (
    <div className="py-3 border-b border-border-soft">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
          ...
          return (
            <g key={`py${i}`}>
              <line
                ...
                strokeWidth={0.5}
              />
              <text x={padL - 4} y={y + 3} fontSize={9} fill="#22d3ee" textAnchor="end">
                {label}
              </text>
            </g>
          );
        ...
        {cYLabels.map(([_val, label], i) => {
          ...
          return (
            <g key={`cy${i}`}>
              <text x={W - padR + 4} y={y + 3} fontSize={9} fill="#fb923c" textAnchor="start">
                {label}
              </text>
            </g>
          );
...
export function StatsPanel({
  ...
  const executorSteps = stepHistory.filter(
    (s) => s.source === "main" || s.source === "executor" || !s.source,
  );
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      {!hasAnyData ? (
        ...
        <div className="flex flex-col gap-0 p-3 overflow-y-auto">
          ...
          {sessionExpanded && (
            <div className="text-[10px] text-fg-faint text-center -mt-2 mb-1">▲ 点击收起明细</div>
          )}
          {!sessionExpanded && (
            <div className="text-[10px] text-fg-faint text-center -mt-2 mb-1">▼ 点击展开明细</div>
          )}
          ...
          {(perTurnPlannerUsage || perTurnExecutorUsage || perTurnSubUsage) && (
            <>
              ...
              >
                <StatsTable
                  title={`本轮 (${turnSteps?.length || 0}步)`}
                  planner={turnPlanner}
                  executor={turnExecutor}
                  sub={turnSub}
                  total={turnTotal}
                  collapsed={!turnExpanded}
                />
              </div>
              {turnExpanded && (
                <div className="text-[10px] text-fg-faint text-center -mt-2 mb-1">
                  ▲ 点击收起明细
                </div>
              )}
              {!turnExpanded && (
          ...
          {lastStep && (
            <div className="py-3 border-b border-border-soft">
              <div className="text-[10px] font-semibold text-fg-faint uppercase tracking-wider mb-2">
                ...
                {lastStep.source && (
                  <span
                    className={`ml-2 text-[9px] px-1 rounded ${lastStep.source === "subagent" ? "bg-warn-soft text-warning" : lastStep.source === "planner" ? "bg-accent-soft/50 text-accent/80" : "bg-accent-soft text-accent"}`}
                  >
                    {lastStep.source === "subagent"
                      ? "子代理"
                      : lastStep.source === "planner"
                        ? "规划模型"
                        : "执行模型"}
                  </span>
                )}
              ...
              {lastStep.prompt > 0 &&
                (() => {
                  ...
                  return (
                    <div className="flex items-baseline gap-2">
                      <span className={`text-xl font-bold tabular-nums ${hitRateColor(rate)}`}>
                        {rate.toFixed(2)}%
                      </span>
                      <span className="text-[10px] text-fg-faint tabular-nums">
                        {tk(lastStep.cacheHit)} 命中 / {tk(lastStep.cacheMiss)} 未命中
                      </span>
                    </div>
                  );
                })()}
          ...
          <HitRateTrend
            ...
            title="命中率趋势 · Hermes"
            color="var(--accent)"
            callCount={plannerSteps.length}
          />

          {/* ── 命中率趋势（执行）── */}
          <HitRateTrend
            steps={executorSteps}
            title="命中率趋势 · Hephaestus"
            color="#3b82f6"
            callCount={executorSteps.length}
          />

          {/* ── 命中率趋势（子代理）── */}
          <HitRateTrend
            steps={subSteps}
            title={`命中率趋势 · ${subagentModel || "子代理"}`}
            color="var(--warn)"
            callCount={subSteps.length}
          />
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Object.entries(toolCounts)
                      ...
                      .map(([name, count]) => (
                        <span
                          key={name}
                          className="inline-flex items-center gap-1 bg-bg-elev border border-border-soft rounded px-1.5 py-0.5 text-[11px]"
                        >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Object.entries(skillCounts)
                      ...
                      .map(([name, count]) => (
                        <span
                          key={name}
                          className="inline-flex items-center gap-1 bg-accent-soft border border-accent/20 rounded px-1.5 py-0.5 text-[11px]"
                        >
          ...
          <div className="flex justify-end pb-1">
            <button
              className="text-[10px] px-1.5 py-0.5 border border-border-soft rounded bg-transparent text-fg-faint cursor-pointer hover:text-err hover:border-err transition-colors"
              onClick={clearData}
              title="清空统计"
            >
              清空统计
