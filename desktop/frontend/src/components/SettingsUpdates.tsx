  ...
  return (
    <section className="mb-3">
      ...
      {status.kind === "upToDate" && (
        <div className="text-fg-faint text-[10px] mt-1 px-1">{t("updater.upToDate")}</div>
      )}
      {status.kind === "available" && (
        <>
          <div className="flex items-center gap-3 mb-2.5">
            <span className="text-fg-dim text-[13px] shrink-0">
              {t("updater.available", { v: status.info.latest })}
            </span>
            <span className="flex-1" />
          ...
          {!status.info.canSelfUpdate && (
            <div className="text-fg-faint text-[10px] mt-1 px-1">{t("updater.macHint")}</div>
          )}
        </>
      )}
      {status.kind === "downloading" && (
      ...
      {status.kind === "verifying" && (
        <div className="text-fg-faint text-[10px] mt-1 px-1">{t("updater.verifying")}</div>
      )}
      {status.kind === "applying" && (
        <div className="text-fg-faint text-[10px] mt-1 px-1">{t("updater.applying")}</div>
      )}
      {status.kind === "done" && (
        <div className="text-fg-faint text-[10px] mt-1 px-1">{t("updater.done")}</div>
      )}
      {status.kind === "error" && (
