...
export const ToolGroup = memo(function ToolGroup({
  ...
  const allSubjects = tools.map((t) => subjectOf(t.name, t.args)).filter((s): s is string => !!s);
  ...
  return (
    <div className="my-1 border border-border-soft rounded-lg overflow-hidden bg-bg-elev/30">
      ...
      >
        ...
        <span className={`text-fg-faint font-mono ${compact ? "text-[10px]" : "text-[11px]"}`}>
          × {tools.length}
        </span>
        {subjects.length > 0 && (
          <span
            className={`text-fg-faint truncate ml-1 ${compact ? "text-[10px]" : "text-[11px]"}`}
          >
            {subjects.join(", ")}
...
export type GroupItem =
  { kind: "item"; item: Item } | { kind: "group"; id: string; name: string; tools: ToolItem[] };
...
export function scanGroups(items: Item[]): GroupItem[] {
  ...
  while (i < items.length) {
    ...
    let j = i + 1;
    while (j < items.length && items[j].kind === "tool" && (items[j] as ToolItem).name === t.name) {
      j++;
    }
