  ...
  switch (e.kind) {
    case "turn_started":
      return {
        ...
        turnTokens: 0,
        perTurnUsage: null,
        perTurnPlannerUsage: undefined,
        perTurnExecutorUsage: undefined,
      ...
      return {
        ...
        items: [
          ...s.items,
          {
            kind: "assistant",
            id,
            text: e.kind === "text" ? delta : "",
            reasoning: e.kind === "reasoning" ? delta : "",
            streaming: true,
          },
        ],
        currentAssistant: id,
        lastAssistantIdx: newIdx,
      };
    }
    case "message": {
      ...
      if (!needNew) {
        ...
        next[idx] = {
          ...it,
          text: e.text ?? it.text,
          reasoning: e.reasoning ?? it.reasoning,
          streaming: false,
        };
    ...
    case "tool_dispatch": {
      ...
      if (idx >= 0) {
        ...
        return { ...s, currentAssistant: undefined, items: next };
      }
      return {
        ...s,
        seq: s.seq + 1,
    ...
    case "tool_result": {
      ...
      if (idx < 0) {
        for (let i = next.length - 1; i >= 0; i--) {
          const cand = next[i];
          if (cand.kind === "tool" && (cand as any).status === "running") {
            idx = i;
            break;
          }
        }
    ...
    case "usage": {
        ...
        s.perTurnUsage && u
          ? {
              ...
              sessionCacheHitTokens:
                u.sessionCacheHitTokens > 0
                  ? u.sessionCacheHitTokens
                  : (s.perTurnUsage?.sessionCacheHitTokens ?? 0),
              sessionCacheMissTokens:
                u.sessionCacheMissTokens > 0
                  ? u.sessionCacheMissTokens
                  : (s.perTurnUsage?.sessionCacheMissTokens ?? 0),
      ...
      const prevPlanner = s.perTurnPlannerUsage,
        prevExecutor = s.perTurnExecutorUsage,
        prevSub = s.perTurnSubUsage;
      const accSrc = (prev?: WireUsage, cur?: WireUsage) =>
        !cur
          ? prev
          : !prev
            ? cur
            : {
                ...
                sessionCacheHitTokens:
                  cur.sessionCacheHitTokens > 0
                    ? cur.sessionCacheHitTokens
                    : prev.sessionCacheHitTokens,
                sessionCacheMissTokens:
                  cur.sessionCacheMissTokens > 0
                    ? cur.sessionCacheMissTokens
                    : prev.sessionCacheMissTokens,
                costUsd: (prev.costUsd ?? 0) + (cur.costUsd ?? 0),
              };
      const tagged = u ? { ...u } : undefined;
      const steps = tagged ? [...s.turnSteps, tagged] : s.turnSteps;
      return {
        ...s,
        usage: tagged,
        perTurnUsage: acc,
        perTurnPlannerUsage: accSrc(prevPlanner, isPlanner ? u : undefined),
        perTurnExecutorUsage: accSrc(prevExecutor, isExecutor ? u : undefined),
        perTurnSubUsage: accSrc(prevSub, isSub ? u : undefined),
        turnSteps: steps,
        context: { ...s.context, used },
        turnTokens: s.turnTokens + (tagged?.completionTokens ?? 0),
      };
    }
    case "notice":
      return {
    ...
    case "turn_done": {
      ...
      const finalItems: Item[] = e.err
        ? [...finalized, { kind: "notice", id: `e${s.seq}`, level: "warn", text: e.err }]
        : finalized;
      const st =
        s.usage?.totalTokens != null && s.usage.totalTokens > 0
          ? s.sessionTotal + s.usage.totalTokens
          : s.sessionTotal;
...
function reducer(s: ControllerState, a: Action): ControllerState {
  switch (a.type) {
    ...
    case "tcca":
      return { ...s, tcca: a.report };
    case "history": {
      const visible = a.messages.filter(
        (m) => (m.role === "user" || m.role === "assistant") && m.content.trim() !== "",
      );
      const lastIdx = visible.reduceRight(
        (acc, m, i) => (acc >= 0 ? acc : m.role === "assistant" ? i : -1),
        -1,
      );
      return {
        ...s,
        items: visible.map((m, i) =>
          m.role === "user"
            ? ({ kind: "user", id: `h${i}`, text: m.content } as Item)
            : ({
                kind: "assistant",
                id: `h${i}`,
                text: m.content,
        ...
        seq: s.seq + visible.length,
        lastAssistantIdx: lastIdx,
      };
    }
    case "clearApproval":
...
export const useStore = create<ControllerState>()(
  (set) =>
    ({ ...initialState, _dispatch: (a: Action) => set((s) => reducer(s, a)) }) as ControllerState,
);
...
export function useController() {
  ...
  useEffect(() => {
    const off = onEvent((e) => {
      ...
      if (e.kind === "turn_done") {
        app
          .ContextUsage()
          .then((c) => dispatch({ type: "context", context: c }))
          .catch(() => {});
        app
          .Balance()
        ...
        app
          ...
          .then((raw) => {
            try {
              dispatch({ type: "tcca", report: JSON.parse(raw) as TCCAReport });
            } catch {}
          })
          .catch(() => {});
      }
      if (e.kind === "turn_done" || e.kind === "notice") {
        app
          .Jobs()
          .then((j) => dispatch({ type: "jobs", jobs: j }))
          .catch(() => {});
      }
    });
  ...
  const approve = useCallback(
    (id: string, allow: boolean, session: boolean) => {
  ...
  const setPermLevel = useCallback((level: string) => {
  ...
  const listSessions = useCallback(
    (): Promise<SessionMeta[]> => app.ListSessions().catch(() => []),
    [],
  );
  const resumeSession = useCallback(
    async (path: string) => {
      const ms = await app.ResumeSession(path).catch(() => [] as HistoryMessage[]);
      dispatch({ type: "reset" });
  ...
  const pickWorkspace = useCallback(async (): Promise<string> => {
    const p = await app.PickWorkspace().catch(() => "");
    if (p) {
      dispatch({ type: "reset" });
      try {
        dispatch({ type: "meta", meta: await app.Meta() });
  ...
  const switchWorkspace = useCallback(
    async (path: string): Promise<string> => {
      const n = await app.SwitchWorkspace(path).catch(() => "");
      if (n) {
        dispatch({ type: "reset" });
        try {
          dispatch({ type: "meta", meta: await app.Meta() });
  ...
  const fetchMemory = useCallback(
    (): Promise<MemoryView> =>
      app
        .Memory()
        .catch(
          () => ({ docs: [], facts: [], scopes: [], storeDir: "", available: false }) as MemoryView,
        ),
    [],
  );
  ...
  const updateFact = useCallback(async (name: string, body: string) => {
    await app.UpdateFact(name, body).catch(() => {});
  }, []);
  const changeFactType = useCallback(async (name: string, typ: string) => {
    await app.ChangeFactType(name, typ).catch(() => {});
  }, []);
  const rewind = useCallback(
    async (turn: number, scope: string) => {
      ...
      const ms = await app.History().catch(() => [] as HistoryMessage[]);
      dispatch({ type: "reset" });
      if (ms.length) dispatch({ type: "history", messages: ms });
      app
        .ContextUsage()
